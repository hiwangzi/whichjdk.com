...
为了构建并运行 Java 应用，需要用到实现了 Java Platform, Standard Edition ("Java SE") 规范的 Java 编译器、Java 运行时库及 Java 虚拟机。
...
[OpenJDK](https://openjdk.java.net) 是对 Java SE 规范的开源参考实现，但它只提供源代码。
不同的供应商为许多受支持的平台提供了二进制发行版。
这些发行版的协议、商业支持、支持的平台以及更新频率都不一样。
...
✅ 推荐：使用 [Adoptium Eclipse Temurin 21](#adoptium-eclipse-temurin) 并且确保你本地的版本和 CI 及生产环境版本一致。
